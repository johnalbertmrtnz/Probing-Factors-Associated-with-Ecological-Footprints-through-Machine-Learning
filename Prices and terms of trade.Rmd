---
title: "Prices and Terms of Trade"
author: "Alberto"
date: "2023-07-02"
output:
...
library(car)
library(rcompanion)
df <- read.csv("4 prices and terms of trade.csv")
...
#Standardization
```{r}
preProcess_params <- preProcess(train_dataset[2:6], method = "scale")
train_df_S <- predict(preProcess_params, train_dataset[2:6])
str(train_df_S)
preProcess_params <- preProcess(test_dataset[2:6], method = "scale")
test_df_S <- predict(preProcess_params, test_dataset[2:6])
summary(test_df_S)
#Algorithm screening
...
### ***Feature Selection***
```{r}
fit.glm2<- train(Class~ Consumer.PI + Import.UVI + Net.barter.terms , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
...
```
```{r}
fit.glm2<- train(Class~ Consumer.PI + Import.UVI , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
predictions <- predict(fit.glm2, test_dataset)
test_results <- confusionMatrix(predictions, test_df_S$Class, mode = "everything")
test_results
...
```
```{r}
fit.glm2<- train(Class~ Import.UVI , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
predictions <- predict(fit.glm2, test_dataset)
test_results <- confusionMatrix(predictions, test_df_S$Class, mode = "everything")
test_results
...
### **Model Diagnostics**
```{r}
base <- glm(formula = Class~ Import.UVI , family = binomial, data = train_df_S)
nagelkerke(base)
```
```{r}
