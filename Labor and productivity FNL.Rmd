...
library(car)
library(rcompanion)
df <- read.csv("labor and productivity.csv")
...
### ***Feature Selection***
```{r}
fit.glm2<- train(Class~ GDP.employed + Industry + Agriculture + Unemployment , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
...
```
```{r}
fit.glm2<- train(Class~ GDP.employed + Industry + Agriculture , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
predictions <- predict(fit.glm2, test_dataset)
test_results <- confusionMatrix(predictions, test_df_S$Class, mode = "everything")
test_results
...
```
```{r}
fit.glm2<- train(Class~ GDP.employed + Industry , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
predictions <- predict(fit.glm2, test_dataset)
test_results <- confusionMatrix(predictions, test_df_S$Class, mode = "everything")
test_results
...
```
```{r}
fit.glm2<- train(Class~ GDP.employed , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
predictions <- predict(fit.glm2, test_dataset)
test_results <- confusionMatrix(predictions, test_df_S$Class, mode = "everything")
test_results
...
### **Model Diagnostics**
```{r}
base <- glm(formula = Class~ GDP.employed + Industry , family = binomial, data = train_df_S)
vif(base)
nagelkerke(base)
```
```{r}
