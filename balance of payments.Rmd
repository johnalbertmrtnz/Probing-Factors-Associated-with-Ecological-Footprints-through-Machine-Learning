---
title: 'Economy: Balance of Payments'
author: "Alberto"
date: "2023-07-01"
output:
...
library(car)
library(rcompanion)
df <- read.csv("balance of payments.csv")
...
### ***Feature Selection***
```{r}
fit.glm2<- train(Class~ CAB + IVI + EVI + PR , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
...
```
```{r}
fit.glm2<- train(Class~ CAB + IVI + EVI , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
predictions <- predict(fit.glm2, test_dataset)
test_results <- confusionMatrix(predictions, test_df_S$Class, mode = "everything")
test_results
...
```
```{r}
fit.glm2<- train(Class~ CAB + IVI , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
predictions <- predict(fit.glm2, test_dataset)
test_results <- confusionMatrix(predictions, test_df_S$Class, mode = "everything")
test_results
...
```
```{r}
fit.glm2<- train(Class~ CAB , data=train_df_S, method="glm", metric=metric, trControl=control)
fit.glm2
predictions <- predict(fit.glm2, test_dataset)
test_results <- confusionMatrix(predictions, test_df_S$Class, mode = "everything")
test_results
...
### **Model Diagnostics**
```{r}
base <- glm(formula = Class~ CAB , family = binomial, data = train_df_S)
nagelkerke(base)
```
```{r}
